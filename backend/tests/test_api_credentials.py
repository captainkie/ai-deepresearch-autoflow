...
async def test_create_lists_masked_and_never_returns_plaintext(auth_client):
    ...
    listed = await auth_client.get("/api/v1/admin/credentials")
...
async def test_revoke_flips_status_and_audits(auth_client):
    ...
    revoke = await auth_client.post(f"/api/v1/admin/credentials/{created['id']}/revoke")
    assert revoke.status_code == 200

    creds = (await auth_client.get("/api/v1/admin/credentials")).json()["credentials"]
    assert creds[0]["status"] == "revoked"

    audit_resp = await auth_client.get("/api/v1/admin/audit")
...
async def test_revoke_unknown_returns_404(auth_client):
    assert (await auth_client.post("/api/v1/admin/credentials/nope/revoke")).status_code == 404


async def test_config_available_reflects_vaulted_provider(auth_client):
    before = (await auth_client.get("/api/v1/config")).json()["llm"]["available"]
    ...
    after = (await auth_client.get("/api/v1/config")).json()["llm"]["available"]
...
async def test_rotate_master_key_bumps_version(auth_client):
    ...
    resp = await auth_client.post(
        "/api/v1/admin/credentials/rotate", json={"new_master_key": keys.generate_master_key()}
    )
...
async def test_rotate_rejects_bad_key(auth_client):
    resp = await auth_client.post(
        "/api/v1/admin/credentials/rotate", json={"new_master_key": "not-base64!!"}
    )
