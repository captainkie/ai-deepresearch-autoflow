...
async def _bootstrap(client):
    ...
    await client.post("/api/setup", json=SUPERADMIN)
...
async def test_register_login_me_flow(client):
    ...
    assert (
        await client.get("/api/auth/me", headers={"Authorization": "Bearer nope"})
    ).status_code == 401
...
async def test_login_wrong_password_rejected(client):
    ...
    resp = await client.post(
        "/api/auth/login", json={"email": "boss@example.com", "password": "wrong"}
    )
...
async def test_login_then_refresh_rotates(client):
    ...
    login = await client.post(
        "/api/auth/login", json={"email": "boss@example.com", "password": "supersecret1"}
    )
