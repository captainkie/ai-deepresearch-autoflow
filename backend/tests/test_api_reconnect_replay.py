...
async def test_reconnect_replays_full_history(auth_client):
    run_id = await _create_run(auth_client)

    # First connection runs the job to completion.
    live = await collect_stream(auth_client, run_id)
    ...
    replay = await collect_stream(auth_client, run_id)
...
async def test_repeated_reconnect_is_stable(auth_client):
    run_id = await _create_run(auth_client)
    await collect_stream(auth_client, run_id)

    first = await collect_stream(auth_client, run_id)
    second = await collect_stream(auth_client, run_id)
