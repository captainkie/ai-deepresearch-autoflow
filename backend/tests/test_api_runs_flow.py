...
async def _create_run(client, **body):
    payload = {
...
async def test_create_returns_run_id(client):
    run_id = await _create_run(client)
    assert isinstance(run_id, str) and run_id


async def test_stream_event_order_and_report(client):
    run_id = await _create_run(client)
    events = await collect_stream(client, run_id)
...
async def test_detail_after_completion(client):
    run_id = await _create_run(client)
...
async def test_list_runs_newest_first(client):
    first = await _create_run(client, query="first topic")
    second = await _create_run(client, query="second topic")
...
async def test_stream_unknown_run_404(client):
    resp = await client.get("/api/runs/does-not-exist/stream")
