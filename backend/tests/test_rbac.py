...
async def _register(client, email: str) -> dict:
    resp = await client.post(
        "/api/v1/auth/register", json={"email": email, "name": email, "password": "password1"}
    )
    return resp.json()


async def test_rbac_matrix(client):
    setup = (await client.post("/api/v1/setup", json=SUPER)).json()
    ...
    assert (
        await client.patch(
            f"/api/v1/admin/users/{viewer['user']['id']}",
            json={"role": "viewer"},
            headers=_h(super_tok),
        )
    ).status_code == 200
    ...
    assert (await client.post("/api/v1/runs", json={"query": "x"})).status_code == 401
    assert (await client.get("/api/v1/admin/credentials")).status_code == 401

    # --- viewer: read-only, cannot create runs ---
    assert (
        await client.post("/api/v1/runs", json={"query": "x"}, headers=_h(viewer_tok))
    ).status_code == 403
    ...
    assert (
        await client.get("/api/v1/admin/credentials", headers=_h(member_tok))
    ).status_code == 403
    assert (
        await client.post("/api/v1/config", json={"llm_provider": "mock"}, headers=_h(member_tok))
    ).status_code == 403

    # --- ownership: a different member can't read the run (hidden as 404) ---
    other = await _register(client, "other@example.com")
    assert (
        await client.get(f"/api/v1/runs/{member_run}", headers=_h(other["access_token"]))
    ).status_code == 404
    assert (
        await client.get(f"/api/v1/runs/{member_run}", headers=_h(member_tok))
    ).status_code == 200
    assert (
        await client.get(f"/api/v1/runs/{member_run}", headers=_h(admin_tok))
    ).status_code == 200

    # --- admin manages creds but cannot rotate the master key (superadmin only) ---
    assert (await client.get("/api/v1/admin/credentials", headers=_h(admin_tok))).status_code == 200
    assert (
        await client.post(
    ...
    assert (
        await client.patch(
            f"/api/v1/admin/users/{root_id}", json={"disabled": True}, headers=_h(admin_tok)
        )
    ).status_code == 403
    ...
    assert (await client.get("/api/v1/auth/me", headers=_h(member_tok))).status_code == 401
    assert (
        await client.post("/api/v1/runs", json={"query": "x"}, headers=_h(member_tok))
    ).status_code == 401
